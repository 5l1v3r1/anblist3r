...
# coding: utf-8
# anblist3r
# By Joel Indra - github.com/anonre
...
def logo():
 ...
 /_/   \_\_| \_|\___/|_| \_|_| \_\_____|
  ...
  Coded By [ Anonre ] github.com/anonre
			                  """
...
#percobaan
class NaverEnum(enumratorBaseThreaded):
    def __init__(self, domain, subdomains=None, q=None, silent=False, verbose=True):
        subdomains = subdomains or []
        base_url = "https://search.naver.com/search.naver?where=nexearch&sm=top_hty&fbm={page_no}&ie=utf8&query={query}"
        self.engine_name = "Naver"
        self.MAX_DOMAINS = 10
        self.MAX_PAGES = 0
        super(NaverEnum, self).__init__(base_url, self.engine_name, domain, subdomains, q=q, silent=silent,
                                        verbose=verbose)
        self.q = q
        return
...
class DaumEnum(enumratorBaseThreaded):
    def __init__(self, domain, subdomains=None, q=None, silent=False, verbose=True):
        subdomains = subdomains or []
        base_url = "https://search.daum.net/search?w=tot&DA=YZR&t__nil_searchbox=btn&sug=&{page_no}=&sq=&o=&q={query}"
        self.engine_name = "Daum"
        self.MAX_DOMAINS = 10
        self.MAX_PAGES = 0
        super(DaumEnum, self).__init__(base_url, self.engine_name, domain, subdomains, q=q, silent=silent,
                                        verbose=verbose)
        self.q = q
        return
...
def main(domain, threads, savefile, ports, silent, verbose, enable_bruteforce, engines, quiet):
    ...
    if engines is None:
        chosenEnums = [
            ...
            CrtSearch, PassiveDNS, NaverEnum, DaumEnum
        ]
    else:
